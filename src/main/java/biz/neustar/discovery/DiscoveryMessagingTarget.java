...
public class DiscoveryMessagingTarget extends AbstractMessagingTarget {

	private static final Logger log = LoggerFactory.getLogger(DiscoveryMessagingTarget.class);
	...
	private AddressHandler addressHandler = new AbstractContextHandler() {
		...
		public void executeGetOnAddress(XDI3Segment targetAddress, GetOperation operation, MessageResult messageResult, ExecutionContext executionContext) throws Xdi2MessagingException {
			...
			Resolver resolver = DiscoveryMessagingTarget.this.proxy.getResolver();
			...
			XdiAttributeCollection uriXdiAttributeCollection = cloudNumberXdiPeerRoot.getXdiAttributeCollection(XRI_URI, true);
			...
			for (Entry<String, List<String>> uriMapEntry : uriMap.entrySet()) {
				...
				for (String uri : uriList) {
					...
					XdiAttributeCollection typeXdiAttributeCollection = cloudNumberXdiPeerRoot.getXdiEntitySingleton(typeXdiEntitySingletonArcXri, true).getXdiAttributeCollection(XRI_URI, true);
